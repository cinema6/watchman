...
BeeswaxMiddleware.prototype.upsertCampaignActiveLineItems = function(data){
    ...
    function findBeeswaxCreatives(req) {
        return self.beeswaxApi.creatives.query({ 
            advertiser_id : req.beeswaxCampaign.advertiser_id,
            active : true 
        })
        .then(function(result){
            if (!result.payload || !result.payload.length){
                return q.reject(new Error('No active creatives found for campaign'));
            }
            req.beeswaxCreatives = result.payload;
            return req;
        });
    }
    ...
    function createLineItem(req) {
        ...
        function initLineItem(data){
            return self.beeswaxApi.lineItems.create({
                ...
                line_item_budget: data.campaign.targetUsers * data.multiplier,
                budget_type: 1,
            ...
            .then(function(result){
                data.beeswaxLineItems.unshift(result.payload);
                return data;
            });
        }
        ...
        function assignCreatives(data) {
            return q.all(data.beeswaxCreatives.map(function(creative){
                return self.beeswaxApi.creativeLineItems.create({
                    ...
                    line_item_id:   data.beeswaxLineItems[0].line_item_id,
                    active:         true
                });
            }))
            .then(function(result){
        ...
        function activateLineItem(data){
            return self.beeswaxApi.lineItems.edit(
                data.beeswaxLineItems[0].line_item_id, { active: true })
            .then(function(result){
                data.beeswaxLineItems[0] = result;
                return data;
            });
        }
    ...
    function updateExistingLineItem(req) {
        var newBudget = req.campaign.targetUsers * req.multiplier;
        self.log.error('Manual Beeswax Update required for Campaign %1 (%2) line items. ' +
            'New impressions target is %3', req.campaign.id, req.beeswaxCampaign.campaign_id, 
            newBudget);
        return req;
    }
...
BeeswaxMiddleware.prototype.adjustCampaignBudget = function(campaign,amount){
    ...
    return self.beeswaxApi.campaigns.find(
        ...
        .spread(function(beeswaxCampaign,response) {
            var updatedBeeswaxCampaign = response.payload;
            self.log.info(
            ...
            return [beeswaxCampaign, updatedBeeswaxCampaign];
        });
};
