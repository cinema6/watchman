...
function BeeswaxMiddleware(beeswaxConfig, cwrxConfig, opts) {
    ...
    this.orgsEndpoint = url.resolve(cwrxConfig.api.root, cwrxConfig.api.orgs.endpoint);
    this.paymentPlansEndpoint = url.resolve(cwrxConfig.api.root,
        cwrxConfig.api.paymentPlans.endpoint);
...
BeeswaxMiddleware.prototype.checkWithinCampaignLimit = function (campaign) {
    const releventStatuses = ld.values(enums.Status)
        .filter(value => value !== enums.Status.Canceled && value !== enums.Status.Deleted);

    return Promise.all([
        this.cwrxRequest.get({
            url: `${this.orgsEndpoint}/${campaign.org}`
        }),
        this.cwrxRequest.get({
            url: this.campaignsEndpoint,
            qs: {
    ...
    ]).then(ld.spread((orgResult, campaignResult) => {
        return this.cwrxRequest.get({
            url: `${this.paymentPlansEndpoint}/${orgResult[0].paymentPlanId}`
        }).then(ld.spread(paymentPlan => {
            if (campaignResult[0].length > paymentPlan.maxCampaigns) {
                throw new Error('Campaign limit has been reached');
            }
        }));
    }));
};
...
BeeswaxMiddleware.prototype.reactivateCampaign = function (campaign) {
    ...
    return this.checkWithinCampaignLimit(campaign).then(() => {
