    ...
    return self.cwrxRequest.get({
        ...
        return (function() {
            ...
            return self.beeswaxApi.advertisers.create({
                ...
                self.log.info('First attempt to create advertiser failed with: %1',
                    err.message);
                return self.beeswaxApi.advertisers.create({
                    advertiser_name : advertiser.name + ' (' + advertiser.id + ')',
                    alternative_id  : advertiser.id,
                    notes : 'Created by Watchman!',
                    active : true
                }).then(function(result){
...
BeeswaxMiddleware.prototype.createCreatives = function(req) {
    ...
    return q.all(req.placements.map(function(placement){
        ...
        var creative = {
            ...
            creative_content     : {},
            pixels               : [self.formatPixelUrl(placement)],
            creative_attributes : {
...
BeeswaxMiddleware.prototype.adjustCampaignBudget = function(campaign,amount){
    const beeswaxCampaignId = (
        ld.get(campaign, 'externalIds.beeswax') ||
        ld.get(campaign, 'externalCampaigns.beeswax.externalId')
    );
    var self = this;

    return self.beeswaxApi.campaigns.find(beeswaxCampaignId)
        .then(function(response){
            var beeswaxCampaign = response.payload;

            if (!beeswaxCampaign) {
                throw new Error(`Couldn't find beeswax campaign(${beeswaxCampaignId}).`);
            }

            if (amount < 0) {
                self.log.info('Skip Budget Decrease for Campaign %1 (%2)',
                    campaign.id, beeswaxCampaign.campaign_id
                );
                return [beeswaxCampaign, response];
            }
