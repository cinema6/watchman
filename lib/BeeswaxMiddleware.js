...
BeeswaxMiddleware.prototype.upsertCampaignActiveLineItems = function(data){
    ...
    function findBeeswaxCampaign(req) {
        return self.beeswaxApi.campaigns.find(req.campaign.externalIds.beeswax)
            .then(function(result){
                req.beeswaxCampaign = result.payload;
                return req;
            });
    }
    ...
    function findBeeswaxLineItems(req){


    }

    function init(d) {
        if (!d || !d.campaign || !d.startDate || !d.endDate) {
            throw new Error('Object containing a campaign, startDate and endDate is required.');
        }
        return { campaign : d.campaign, startDate : d.startDate, endDate : d.endDate };
    }
