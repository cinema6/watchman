...
const enums = require('cwrx/lib/enums');
...
module.exports = config => {
    ...
    */
    ...
    function checkWithinCampaignLimit(orgId) {
        const releventStatuses = _.values(enums.Status)
            .filter(value => value !== enums.Status.Canceled && value !== enums.Status.Deleted);

        return Promise.all([
            request.get({
                url: `${orgsEndpoint}/${orgId}`
            }),
            request.get({
                url: campaignsEndpoint,
                qs: {
        ...
        ]).then(_.spread((orgResult, campaignResult) => {
            return request.get({
                url: `${paymentPlansEndpoint}/${orgResult[0].paymentPlanId}`
            }).then(_.spread(paymentPlan => {
    ...
    return { rebalance, checkWithinCampaignLimit };
};
